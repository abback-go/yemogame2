# 월드맵 마스터 문서 — 대협곡 세계 (v1, 2026-07-15)

> 생성원: `docs/world/regions/*.json` (지역별 원본) → `tools/worldmap_build.py`가 검증·병합.
> 수정은 지역 JSON에서 하고 스크립트를 다시 돌릴 것. 이 문서와 `godot/data/world_map.json`은 산출물.
...
| sch_clock_face | **문자반 층** `warm` | field | 시계탑 | 시계 문자반 뒤편. 유리 너머로 협곡과 건너편 폐교가 한눈에 들어온다. | →sch_clock_gears; →sch_clock_peak(기류 승월) |
| sch_clock_peak | **시계탑 꼭대기** | transit | 시계탑 | 세계의 바람이 지나가는 첨탑 끝. 여기서 하늘의 기류에 오른다 — 최후반의 문. | →sch_clock_face; →sky_clock_updraft(기류 승월) |
...
- 루프: 낙하 수직굴로 한 번에 내려가고, 벽차기 질주로 벽 하부에서 되올라오는 수직 루프
- 루프: 간이 곤돌라역 개통 후: 중단↔학교/마을 패스트트래블 루프

| id | 방 | 유형 | 구획 | 컨셉 | 연결 |
|---|---|---|---|---|---|
| cyn_east_rim | **동쪽 벼랑 끝** | transit | 협곡 상부 | 협곡 동쪽 가장자리. 고원의 바람이 밀려 내려오고, 건너편엔 폐교가 마주 서 있다. | →cyn_rim_path; →lig_west_gate(바람 타기); →old_front_gate(기류 승월) |
...
| cyn_drop_shaft | **낙하 수직굴** | transit | 협곡 하부 | 바닥이 보이지 않는 수직굴 — 뛰어내리면 빠르지만, 돌아오는 길은 벽을 달려야 한다. | →cyn_wall_mid; →cyn_wall_low(벽차기 질주)[일방] |
| cyn_wall_low | **어스름 벽 하부** | field | 협곡 하부 | 여기서부터 해가 닿은 적 없는 세계. 이끼가 빛을 대신해 희미하게 빛난다. | →cyn_drop_shaft(벽차기 질주); →cyn_floor_south; →cyn_root_cave(비밀) |
...
### 거인의 광산 (`min`) — 32방
- **정체성**: 화석이 된 태고 거인의 몸을 파 내려가는 광산 — 마력석의 산지이자 세계의 길찾기 미로. 공중 질풍보가 여는 첫 대지역.
...
- **지역 마법**: 융기(가칭) — 발밑에서 돌기둥을 솟구쳐 적을 띄우고 낮은 발판이 된다 — 무겁고 결정적인 지역 마법
...
| min_upper_2 | **수레 나들목** | field | 상부 갱도 | 수레를 타고 갈라지는 갱도들을 익히는 구간 — 이 지역 이동 트릭의 도입부. | →min_rail_st; →min_winch; →min_maze_gate(공중 질풍보) |
...
| min_echo_gallery | **울림 회랑** | field | 상부 갱도 | 소리가 벽을 타고 달리는 회랑. 조용히 걷는 법을 배우는 응용 구간. | →min_dust_hall; →min_vein_1(공중 질풍보) |
...
| min_vein_1 | **굳은 핏줄 광맥** | field | 굳은 핏줄 | 거인의 혈관이 광맥이 된 구간. 붉은 결이 램프 빛에 맥박처럼 일렁인다. | →min_maze_exit; →min_echo_gallery; →min_vein_2(공중 질풍보) |
| min_vein_2 | **맥박 광층** | field | 굳은 핏줄 | 발디딜 곳이 끊겨 공중 질풍보 없이는 건널 수 없는 광층. 벽이 미세하게 두근거린다. | →min_vein_1(공중 질풍보); →min_vein_camp; →min_lower_breach |
...
| min_lower_breach | **무너진 하부** | transit | 굳은 핏줄 | 바닥이 숲으로 꺼져 내린 붕괴 지점 — 뛰어내릴 수는 있지만 돌아올 수는 없다. | →min_vein_2; →for_west_roots(벽차기 질주)[일방] |
| min_crush_arena | **무너지는 홀** | miniboss | 굳은 핏줄 | 천장이 반쯤 내려앉은 홀 — 미니보스 '무너뜨리는 자'가 길을 막는다(웹 프로토타입 계승). | →min_vein_camp; →min_deep_shaft |
| min_deep_shaft | **심부 수직굴** | field | 심장부 | 거인의 심장으로 내려가는 마지막 수직굴. 공중 질풍보로 벽 사이를 지그재그로 딛는다. | →min_crush_arena; →min_heart_gate; →min_gem_hollow(비밀) |
...
### 잠들지 않는 숲 (`for`) — 30방
- **정체성**: 해가 닿은 적 없는 협곡 바닥의 숲 — 빛을 재배하는 사람들의 땅. 추격과 매복의 지역, 벽차기 질주가 연다.
...
- **지역 마법**: 그림자 손(가칭) — 어둠에서 손이 뻗어 적을 붙잡아 세운다 — 추격자를 끊는 결정적 지역 마법, 어스름 장막(가칭) — 짧게 주위 빛을 삼켜 적의 시선을 끊는다
...
| for_west_roots | **서쪽 뿌리 비탈** | transit | 어스름 층 | 광산의 붕괴 지점에서 떨어져 들어오는 뒷문 비탈. 위로는 벽을 달려야 되돌아갈 수 있다. | →for_dusk_2; →min_lower_breach(벽차기 질주) |
...
| for_deep_gate | **심부 문** | transit | 심부 | 씨앗 램프의 빛에만 반응해 열리는 뿌리의 문 — 벽차기 질주로 뿌리 벽을 달려 내려간다. | →for_black_3; →for_deep_1(벽차기 질주) |
...
### 거울의 도시 (`mir`) — 32방
- **정체성**: 태양빛을 거울 배관으로 골목마다 배달하는 사막 도시 — 빛 퍼즐의 지역. 발판 각인이 연다. 로컬 스토리: '태양로가 흐려진다'.
...
- **지역 마법**: 광창(가칭) — 모아 쏘는 태양빛의 창 — 어둠 속성 적에게 결정적, 거울에 반사되어 꺾인다
...
- 루프: 거울 대로에서 그림자 골목으로 일방 낙하 → 뒷계단(벽차기 질주)으로 상층 시가 복귀 (§5.5의 내부 루프)
- 루프: 보스 후 직통 승강기: 태양로 심장↔중앙 광장
- 루프: 지하 수로 수문 개방 → 광산 서단으로 이어지는 외부 루프(월드 루프 ⑤ 보조)

| id | 방 | 유형 | 구획 | 컨셉 | 연결 |
|---|---|---|---|---|---|
| mir_gate_east | **사막 관문** | transit | 관문 | 가도의 끝, 모래바람을 막는 이중문. 거울 각도 기믹이 처음 손에 잡히는 곳. | →roa_desert_west(발판 각인); →mir_gate_plaza |
...
| mir_mirror_ave | **거울 대로** | field | 중앙 | 빛 경로 퍼즐의 응용 구간 — 대로의 거울을 돌려 배관에 빛을 채우면 길이 열린다. | →mir_plaza; →mir_bureau_gate; →mir_high_street(발판 각인); →mir_shadow_alley[일방] |
...
| mir_high_street | **상층 시가** | field | 상층 | 부유한 지붕들의 거리 — 발판 각인으로 허공에 길을 새겨 오르는 수직 시가지. | →mir_mirror_ave(발판 각인); →mir_pipe_cross; →mir_spire_base; →mir_back_stairs |
| mir_pipe_cross | **빛 배관 교차로** | field | 상층 | 도시의 모든 빛이 갈라지는 교차로 — 거울 각도와 발판 각인이 조합되는 최고 난도 퍼즐. | →mir_high_street; →mir_furnace_upper; →mir_pipe_maint(비밀) |
...
| mir_spire_climb | **거울탑 오르기** | challenge | 상층 | 격리된 정밀 플랫포밍 챌린지 — 거울 반사 타이밍에 맞춰 발판을 새기며 오른다. | →mir_spire_base; →mir_spire_top |
...
| mir_back_stairs | **뒷계단** | transit | 그림자 골목 | 골목에서 상층 시가로 벽을 타고 오르는 지름길 — 아래에서 위로 여는 내부 루프. | →mir_shadow_court; →mir_high_street(벽차기 질주) |
...
### 번개 목장 (`lig`) — 28방
- **정체성**: 폭풍의 번개를 수확해 병에 담는 고원 목장 — 고난도 액션의 지역. 바람 타기가 연다.
...
| id | 방 | 유형 | 구획 | 컨셉 | 연결 |
|---|---|---|---|---|---|
| lig_west_gate | **고원 오르막 문** | transit | 초지 | 협곡 동쪽 벼랑에서 고원으로 오르는 바람길 — 바람 타기 없이는 기류를 거슬러 오를 수 없다. | →cyn_east_rim(바람 타기); →lig_meadow_1 |
...
| lig_south_trail | **남쪽 벼랑길** | transit | 마을 | 조수 도시 쪽으로 굽이치며 내려가는 벼랑길 — 바닷바람과 고원 바람이 만나는 곳. | →lig_village; →tid_cliff_north(바람 타기) |
...
| lig_storm_top | **폭풍 위 하늘** | transit | 폭풍의 눈 | 보스 후 — 폭풍이 걷힌 자리에 상승기류의 기둥이 남는다. 부유군도로 오르는 두 번째 문. | →lig_storm_eye; →sky_storm_updraft(기류 승월) |
...
- **지역 마법**: 서리 결속(가칭) — 적의 발을 얼려 그 자리에 묶는다 — 소리 없이 상황을 정지시키는 지역 마법
...
### 바람이 시작되는 곳 (`sky`) — 22방
- **정체성**: 세계의 바람이 시작되는 부유 군도 — 최후반 지역. 기류 승월이 열고, 시계탑과 폭풍 두 개의 하늘길이 닿는다.
...
- **지역 마법**: 기류 부름(가칭) — 그 자리에 상승기류 기둥을 세운다 — 이동과 전투(띄우기)를 겸하는 바람 심화 마법
...
| id | 방 | 유형 | 구획 | 컨셉 | 연결 |
|---|---|---|---|---|---|
| sky_clock_updraft | **시계탑 기류 상륙섬** | transit | 상륙 | 학교 시계탑에서 오르는 기류가 닿는 첫 섬 — 아래로 학교가 장난감처럼 보인다. | →sch_clock_peak(기류 승월); →sky_isle_gate_w |
| sky_storm_updraft | **폭풍 기류 상륙섬** | transit | 상륙 | 걷힌 폭풍 위로 오르는 기류가 닿는 동쪽 섬 — 아직 정전기가 풀잎에 남아 있다. | →lig_storm_top(기류 승월); →sky_isle_gate_e |
...
| sky_wind_shrine | **바람 사당** | event | 바람 정원 | 바람 심화 마법이 잠든 사당 — 기류 부름을 여기서 얻는다. 사당지기는 없고 바람이 대답한다. | →sky_shrine_gate; →sky_spiral |
| sky_spiral | **나선 기류** | field | 높은 하늘 | 조합 구간 — 나선으로 감아 오르는 기류. 발판 각인·바람 타기·기류 승월이 한 호흡에 엮인다. | →sky_current_2; →sky_wind_shrine; →sky_high_reach |
...
### 첫 학교(폐교) (`old`) — 10방
- **정체성**: 협곡 건너편 절벽의 폐교 — 입학 첫날부터 보이지만 갈 수 없는 랜드마크. 기류 승월이 열어 주는 떡밥의 창고.
...
- 루프: 부유군도에서 지붕으로 떨어지는 뒷문(일방)과 정문(기류 승월)의 이중 진입

| id | 방 | 유형 | 구획 | 컨셉 | 연결 |
|---|---|---|---|---|---|
| old_front_gate | **무너진 정문** | transit | 바깥뜰 | 현판의 교명이 바람에 닳아 읽히지 않는다 — 협곡을 건너온 자만 서는 문 앞. | →cyn_east_rim(기류 승월); →old_yard |
...
### 조수 도시 (`tid`) — 10방
- **정체성**: 밀물과 썰물로 길이 바뀌는 항구 도시 — 같은 골목이 물때에 따라 다른 지도가 된다. 발판 각인 연계 소지역.
...
| tid_quay | **부두** `map,shop` | town | 항구 | 도시의 현관 — 지역 지도 입수처. 밧줄과 생선 궤짝, 물때표가 걸린 게시판. | →tid_river_west; →tid_market; →tid_lighthouse_base; →tid_lower_town(발판 각인) |
...
| tid_lower_town | **물에 잠긴 하층** | field | 하층 | 밀물이면 지붕만 남는 옛 시가 — 썰물이면 골목이, 밀물이면 뱃길이 된다. | →tid_quay(발판 각인); →tid_flooded_lane(상태:썰물); →tid_cliff_north |
...
| tid_cliff_north | **북쪽 벼랑길** | transit | 하층 | 바닷바람이 벼랑을 때리는 북행 길 — 고원의 목장까지, 바람을 타는 자만 오른다. | →tid_lower_town; →lig_south_trail(바람 타기) |
...
| kni_training_wall | **수직 훈련벽** | challenge | 마을 | 기사 후보생의 담력 시험 벽 — 벽차기 질주 챌린지 코스로 완벽하다. 완주하면 단원들의 박수. | →kni_watchtower |
...
- 루프: 절벽 밧줄 샛길(벽차기 질주)로 남쪽 가도에 내려가고, 거울의 도시 남문(내부 개방 후)→가도로 되돌아오는 순환
...
| roa_knight_gate | **무너진 벼랑 구간** | field | 서쪽 팔 | 기사단 마을로 가는 가도가 벼랑째 무너진 곳 — 벽을 달릴 수 있다면 지나갈 수 있다. | →roa_rail_junction; →kni_road_west(벽차기 질주) |
...
| roa_desert_west | **사막 관문 앞** | transit | 서쪽 팔 | 거울의 도시 이중문 앞 모래 언덕 — 허공에 발판을 새길 수 있어야 문루에 오른다. | →roa_rail_westend; →mir_gate_east(발판 각인) |
| roa_cliff_drop | **절벽 밧줄 샛길** | transit | 남쪽 팔 | 옛 밀수꾼들이 절벽에 걸어 둔 샛길 — 벽을 달릴 줄 아는 자만 오르내린다. | →roa_rail_junction; →roa_south_high(벽차기 질주) |
| roa_south_high | **남쪽 가도 상단** | field | 남쪽 팔 | 절벽 허리를 감아 도는 가도 — 위로는 거울의 도시 남문, 아래로는 가마 연기가 보인다. | →roa_cliff_drop(벽차기 질주); →roa_south_low; →mir_south_gate(상태:남문 개방) |
