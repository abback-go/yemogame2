...
## 1. 프로젝트 한 줄 `[2026-08-09 현행 컨셉으로 갱신]`

**눈이 가려진 세 짐승 신(여우·늑대·용)을 몸에 담고, 그들 대신 병든 땅을 보러 다니는 인간**의
**2D 사이드뷰 도트 액션 메트로바니아**. (컨셉 정본 = `game-design.md` §1 — ~~성창사~~ 구 컨셉은 폐기)
...
- 브랜치 **`claude/magic-school-game-concept-wchpjn`** 에 커밋·푸시 `[08-09 정정 — 구 브랜치명이 남아 있었음]`. 커밋 메시지는 한국어, 상세히. PR #1이 추적 — 새 PR 금지.
- **변경 후 항상** `node proto/verify-map.mjs`(또는 인게임 `T`)로 **19항목** 통과 확인 + 전 층 렌더 에러 0.
